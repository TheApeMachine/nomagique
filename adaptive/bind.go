...
func bindObserveOne(stage core.Number) func(float64) float64 {
	...
	case *Exponential:
		return func(raw float64) float64 {
			return ObserveEMA(&dynamic.state, raw)
		}
	case *Normalized:
		return func(raw float64) float64 {
			return ObserveDelta(&dynamic.state, raw)
		}
	case *Integrator:
		return func(raw float64) float64 {
			return ObserveAccumulator(&dynamic.state, raw)
		}
	case *Compressor:
		return func(raw float64) float64 {
			return ObserveCompression(&dynamic.state, raw)
		}
	case *Fractional:
		return func(raw float64) float64 {
			return ObserveFracDiff(&dynamic.state, raw)
		}
	case *Dispersion:
		return func(raw float64) float64 {
			return ObserveVariance(&dynamic.state, raw)
		}
	case *Surprise:
		return func(raw float64) float64 {
			return ObserveZScore(&dynamic.state, raw, 0, false)
		}
	case *Impulse:
		return func(raw float64) float64 {
			return ObserveMomentum(&dynamic.state, raw)
		}
	case *Extent:
		return func(raw float64) float64 {
			return ObserveRange(&dynamic.state, raw)
		}
	default:
