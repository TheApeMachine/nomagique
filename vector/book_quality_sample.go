...
type BookQualitySample struct {
	...
	bytes      []byte
	ledger     algorithm.SideFlowLedger
...
func (bookQualitySample *BookQualitySample) ingestBook(state *datura.Artifact) {
	...
	bookQualitySample.applyLevels(
		state, "bids", algorithm.SideBid,
		&frameCancelBid, &frameFillBid, &frameAddBid, &touchCancelBid,
	)

	bookQualitySample.applyLevels(
		state, "asks", algorithm.SideAsk,
	...
	bookQualitySample.ledger.ApplyFlow(
		algorithm.SideBid, frameFillBid, frameCancelBid, smoothing,
	)
	bookQualitySample.ledger.ApplyFlow(
		algorithm.SideAsk, frameFillAsk, frameCancelAsk, smoothing,
	)

	cancelBid, fillBid, cancelAsk, fillAsk, bidDepth, askDepth := bookQualitySample.ledger.Snapshot()
	maxRatio := math.Max(
		algorithm.CancelFillRatio(cancelBid, fillBid),
		algorithm.CancelFillRatio(cancelAsk, fillAsk),
	)
	...
	if lastPrice > 0 && churnGate > 0 {
		...
		sizeThreshold := bookQualitySample.gates.LargeBlockQtyThreshold(
			bookQualitySample.ledger.SideDepth(algorithm.SideBid),
