...
func TestBookQualitySample_Read(t *testing.T) {
	Convey("Given a liquidity vacuum book replay", t, func() {
		...
		Convey("It should emit non-uniform classifier output", func() {
			...
			vacuumScore := datura.Peek[float64](result, "output", "vacuumScore")
			supportScore := datura.Peek[float64](result, "output", "supportScore")

			So(category, ShouldBeGreaterThan, 0)
			So(confidence, ShouldBeGreaterThan, 0)
			So(confidence, ShouldNotAlmostEqual, 1.0/3.0, 0.0001)
			So(bluffScore+vacuumScore+supportScore, ShouldBeGreaterThan, 0)
